load_NETSeq_BAM <- function(bamfiles, bamdir = ".", ngs_mode = "SE", cov_mode = "end",  ss_5p = NULL, ss_3p = NULL, bs = NULL, 
                          pattern = "TGGAATTCTC", trim_names = NULL, expand_names = NULL, remove_split_reads = TRUE, 
                          remove_ss_reads = TRUE, remove_misprimed = TRUE, save_rds = FALSE, filter_tlen = NULL) {
  mat <- matrix(0, nrow = length(bamfiles), ncol = 7)
  for (i in seq_along(bamfiles)) {
    res <- vector("integer", 7)
    bamfile <- bamfiles[[i]]
    message(bamfile); flush.console()
    if (ngs_mode == "SE") {
      data <- readGAlignments(file.path(bamdir, bamfile))
      strand(data) <- ifelse(strand(data) == "+", "-", "+") # flip the strand info (because this is NET-Seq)
    } else if (ngs_mode == "PE") {
      data <- readGAlignmentPairs(file.path(bamdir, bamfile), strandMode = 1)
    }
    ...
    # Filter out reads with insert size above the threshold:
    if (ngs_mode == "PE" && !is.null(filter_tlen) && is.numeric(filter_tlen) && length(filter_tlen) == 1) {
      tlen <- width(granges(data))
      bad_tlen <- tlen > filter_tlen
      data <- data[!bad_tlen]
      res[[6]] <- sum(bad_tlen)
    } else {
      res[[6]] <- 0
    ...
    cov <- convert_GAlignments_to_coverage(data, mode = cov_mode)
    save_GRanges_as_bedGraph(cov, paste0(out_filename, ".bedgraph.gz"))
    message("\t", length(data), " clean reads returned;")
    res[[7]] <- length(data)
    mat[i, ] <- res
  }
  df <- as.data.frame(mat)
  names(df) <- c("Loaded", "Split", "3pSS", "5pSS", "Misprimed", "TLEN", "Returned")
